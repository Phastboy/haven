...
@Component({
  ...
  standalone: true,
  imports: [CardModule, ButtonModule, InputTextModule, FloatLabelModule, MessageModule, FormField],
...
export default class LoginPage implements AfterViewInit {
  ...
  onSubmit(event: Event) {
    ...
    submit(this.magicForm, async () => {
      ...
      try {
        ...
        const { data, error } = await this.api.auth["magic-link"].request.post({
          email: emailValue,
        });
        if (error) {
  ...
  private async handleGoogleCallback(response: any) {
    ...
    try {
      const { data, error } = await this.api.auth.google.login.post({
      ...
      this.messageService.add({
        severity: "error",
        summary: "Error",
        detail: "Network error occurred during Google login",
      });
    }
  }
}
