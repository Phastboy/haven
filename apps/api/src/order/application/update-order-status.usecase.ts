...
import { OrderNotFoundError, UnauthorizedOrderActionError, InvalidOrderStateTransitionError } from '../domain/errors';
...
export class UpdateOrderStatusUseCase {
  ...
  async execute(params: { orderId: string; accountId: string; newStatus: OrderStatus }): Promise<Order> {
    ...
    if (params.newStatus === 'CANCELLED') {
      ...
      if (!isOwner) {
        throw new InvalidOrderStateTransitionError(`Only the offer owner can mark an order as ${params.newStatus}.`);
      }
      if (order.status !== 'PENDING') {
        throw new InvalidOrderStateTransitionError(`Cannot transition from ${order.status} to ${params.newStatus}.`);
      }
    } else if (params.newStatus === 'COMPLETED') {
      if (!isOwner) {
        throw new InvalidOrderStateTransitionError('Only the offer owner can mark an order as COMPLETED.');
      }
      if (order.status !== 'ACCEPTED') {
        throw new InvalidOrderStateTransitionError('Only ACCEPTED orders can be marked as COMPLETED.');
      }
