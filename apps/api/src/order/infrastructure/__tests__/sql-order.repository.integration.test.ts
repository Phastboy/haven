...
describe("SqlOrderRepository Integration", () => {
  ...
  beforeAll(async () => {
    ...
    await db
      .insert(accounts)
      .values({ id: testAccountId1, email: `order-owner-${Date.now()}@example.com` });
    await db
      .insert(users)
      .values({ id: testUserId1, accountId: testAccountId1, username: `orderowner-${Date.now()}` });

    testAccountId2 = randomUUID();
    testUserId2 = randomUUID();
    await db
      .insert(accounts)
      .values({ id: testAccountId2, email: `order-req-${Date.now()}@example.com` });
    await db
      .insert(users)
      .values({ id: testUserId2, accountId: testAccountId2, username: `orderreq-${Date.now()}` });
