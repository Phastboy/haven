...
export class OrderFulfillmentAdapter implements IOrderFulfillmentService {
  async getOrderDetails(orderId: string): Promise<{ id: string; requesterId: string; offerId: string; status: string } | null> {
  ...
  async getOfferTypeAndOwner(offerId: string): Promise<{ offerType: string; ownerId: string } | null> {
