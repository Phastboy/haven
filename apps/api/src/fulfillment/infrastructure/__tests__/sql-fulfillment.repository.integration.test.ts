...
describe('SqlFulfillmentRepository Integration', () => {
  ...
  beforeAll(async () => {
    ...
    await db.insert(accounts).values({ id: testAccountId, email: `fulfillment-owner-${Date.now()}@example.com` });
    await db.insert(users).values({ id: testUserId, accountId: testAccountId, username: `fulfillmentowner-${Date.now()}` });
