...
describe('Fulfillment API E2E', () => {
  ...
  beforeAll(async () => {
    ...
    await db.insert(accounts).values({ id: ownerAccountId, email: `f-owner-${Date.now()}@example.com` });
    await db.insert(users).values({ id: ownerUserId, accountId: ownerAccountId, username: `fowner-${Date.now()}` });
    ...
    await db.insert(accounts).values({ id: requesterAccountId, email: `f-req-${Date.now()}@example.com` });
    await db.insert(users).values({ id: requesterUserId, accountId: requesterAccountId, username: `freq-${Date.now()}` });
  ...
  it('should not allow owner to request a revision', async () => {
    const req = new Request(`http://localhost/api/orders/${testOrderId}/fulfillment/request-revision`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${ownerSessionToken}`
