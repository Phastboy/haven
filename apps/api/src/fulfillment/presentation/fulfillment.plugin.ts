...
import { deliverFulfillmentBodySchema, requestRevisionBodySchema } from '../domain/fulfillment.schema';
...
export const createFulfillmentPlugin = () => {
  ...
  return new Elysia({ prefix: '/orders/:orderId/fulfillment', tags: ['Fulfillment'] })
    .error(FulfillmentNotFoundError, ({ set, error }) => { set.status = 404; return { error: error.message }; })
    .error(UnauthorizedFulfillmentActionError, ({ set, error }) => { set.status = 403; return { error: error.message }; })
    .error(InvalidFulfillmentStateTransitionError, ({ set, error }) => { set.status = 400; return { error: error.message }; })
    .error(RevisionNotApplicableError, ({ set, error }) => { set.status = 400; return { error: error.message }; })
