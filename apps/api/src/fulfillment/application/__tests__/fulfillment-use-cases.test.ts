...
import { InvalidFulfillmentStateTransitionError, RevisionNotApplicableError, UnauthorizedFulfillmentActionError, FulfillmentNotFoundError } from '../../domain/errors';
...
describe('Fulfillment Use Cases', () => {
  describe('DeliverFulfillmentUseCase', () => {
    it('should throw if order is not ACCEPTED', async () => {
      ...
      await expect(usecase.execute({ orderId: 'o1', accountId: 'owner1', autoReviewDays: 3 })).rejects.toThrow(InvalidFulfillmentStateTransitionError);
    });
    ...
    it('should throw if user is not the owner', async () => {
      ...
      const usecase = new DeliverFulfillmentUseCase(repo, service);
      await expect(usecase.execute({ orderId: 'o1', accountId: 'hacker', autoReviewDays: 3 })).rejects.toThrow(UnauthorizedFulfillmentActionError);
    });
    ...
    it('should successfully deliver an accepted order', async () => {
      ...
      const usecase = new DeliverFulfillmentUseCase(repo, service);
      const result = await usecase.execute({ orderId: 'o1', accountId: 'owner1', autoReviewDays: 3 });
  ...
  describe('AcceptFulfillmentUseCase', () => {
    it('should allow requester to accept DELIVERED fulfillment', async () => {
      const repo = {
        getFulfillmentByOrderId: mock(async () => ({ id: 'f1', status: 'DELIVERED', orderId: 'o1' })),
        updateFulfillmentStatus: mock(async (id: string, status: string) => ({ id, status }))
      } as any;
      const service = {
    ...
    it('should throw if non-requester attempts to accept', async () => {
      ...
      const usecase = new AcceptFulfillmentUseCase(repo, service);
      await expect(usecase.execute({ orderId: 'o1', accountId: 'owner1' })).rejects.toThrow(UnauthorizedFulfillmentActionError);
    });
  });
  ...
  describe('RequestRevisionUseCase', () => {
    it('should throw RevisionNotApplicableError if not a SERVICE', async () => {
      ...
      await expect(usecase.execute({ orderId: 'o1', accountId: 'req1', reason: 'broken' })).rejects.toThrow(RevisionNotApplicableError);
    });
  });
