...
describe("SqlDirectoryRepository Integration", () => {
  ...
  beforeAll(async () => {
    ...
    await db
      .insert(accounts)
      .values({ id: testAccountId1, email: `dir1-${Date.now()}@example.com` });
    await db
      .insert(users)
      .values({ id: testUserId1, accountId: testAccountId1, username: `dir1-${Date.now()}` });

    testAccountId2 = randomUUID();
    testUserId2 = randomUUID();
    await db
      .insert(accounts)
      .values({ id: testAccountId2, email: `dir2-${Date.now()}@example.com` });
    await db
      .insert(users)
      .values({ id: testUserId2, accountId: testAccountId2, username: `dir2-${Date.now()}` });
